...
func _ready() -> void:
	if speed <= 0:
		print("Warning: EnergyBall speed is non-positive. Setting to 200.0.")
		speed = 200.0
	if damage <= 0:
		print("Warning: EnergyBall damage is non-positive. Setting to 1.")
		damage = 1
	if lifetime <= 0:
		print("Warning: EnergyBall lifetime is non-positive. Setting to 3.0.")
		lifetime = 3.0
...
func set_speed(new_speed: float) -> void:
	if new_speed <= 0:
		print("Warning: EnergyBall speed cannot be non-positive. Setting to 200.0.")
		speed = 200.0
	else:
		speed = new_speed
...
func set_lifetime(time: float) -> void:
	if time <= 0:
		print("Warning: EnergyBall lifetime cannot be non-positive. Setting to 3.0.")
		lifetime = 3.0
	else:
		lifetime = time
		_lifetime_timer = 0.0
...
func explode() -> void:
	...
	if explosion_scene and explosion_scene.can_instantiate():
		...
		explosion.scale = Vector2(0.5, 0.5) # Smaller explosion
